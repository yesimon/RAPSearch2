...
int main(int argc, char** argv)
{
	...
	bool bLogE = true;
		...
		for (i = 0; i < argc; i ++)
		{
				...
				else if(argv[i][1] == OPTION_INFO_DB[0] && argc > i + 1) // string : info db (if separate)
					{
						szDbInfo = argv[i+1];
					}
	...
	if (bBitsCut == true)
	{
		...
		hs.Process(szDbHash, szDbInfo, szQrFile, szOutFile, nStdout, bEvalue, bLogE, dThr, nMaxAlnPer, nMaxHitPer, nQueryType, bPrintEmpty, bGapExt, bAcc, bHssp, nMinLen, bXml);
...
void printUsage(const char *error)
{
		...
		fprintf(stderr,
						...
						"\t-" OPTION_INFO_DB      " string : database info file (for piping the database file)\n"
						...
						,
						1, 1.0, 0.0, 0, 500, 100, "u", "f", "t", "f", "f", "f", Program, Program, Program, Program
					 );
		fprintf(stderr,
